...
class SmallJavaIndexTest {
	...
	@Test def void testClassesInIndex() {
		...
		'''
			...
			class D { }
		'''.parse(first.eResource.resourceSet).classes.head.assertClassesInIndex("my.first.pack.C, my.second.pack.D")
	}
	...
	@Test def void testVisibleExternalClassesInIndex() {
		...
		first.classes.head.assertExternalClassesInIndex("my.second.pack.D")
		second.classes.head.assertExternalClassesInIndex("my.first.pack.C")
	...
	@Test def void testVisibleExternalClassesContainDuplicatesFromExternalFiles() {
		...
		first.classes.head.assertExternalClassesInIndex("my.first.pack.C, my.first.pack.D")
		second.classes.head.assertExternalClassesInIndex("my.first.pack.C")
	}
	...
	def private assertClassesInIndex(EObject o, String expected) {
		...
		expected.assertEquals(visibleClassesDescriptions.map[it.qualifiedName].join(", "))
	}
	...
	def private assertExternalClassesInIndex(EObject o, String expected) {
		...
		expected.assertEquals(visibleExternalClassesDescriptions.values.map[it.qualifiedName].join(", "))
	}
